...
async def live_proxy(endpoint: str, session_key: str = "latest"):
    ...
    if endpoint not in ("positions", "intervals", "overtakes", "race_control", "weather", "car_data", "pits", "stints", "team_radio"):
...
async def sessions_for_meeting(meeting_key: int):
    ...
    return await safe_cached_get(f"{OPENF1}/sessions?meeting_key={meeting_key}", [], ttl=3600)


@router.get("/positions")
async def positions(session_key: str = "latest"):
    """Get current race positions."""
    return await safe_cached_get(f"{OPENF1}/positions?session_key={session_key}", [], ttl=8)
...
async def pits(session_key: str = "latest"):
    """Get pit stop data."""
    return await safe_cached_get(f"{OPENF1}/pit_stops?session_key={session_key}", [], ttl=30)


@router.get("/stints")
async def stints(session_key: str = "latest"):
    """Get stint data."""
    return await safe_cached_get(f"{OPENF1}/stints?session_key={session_key}", [], ttl=60)


@router.get("/weather")
async def weather(session_key: str = "latest"):
    """Get weather data."""
    return await safe_cached_get(f"{OPENF1}/weather?session_key={session_key}", [], ttl=30)


@router.get("/race_control")
async def race_control(session_key: str = "latest"):
    """Get race control messages."""
    data = await safe_cached_get(f"{OPENF1}/race_control_messages?session_key={session_key}", [], ttl=15)
    return data
...
async def intervals(session_key: str = "latest"):
    """Get interval gaps."""
    return await safe_cached_get(f"{OPENF1}/intervals?session_key={session_key}", [], ttl=8)
...
async def starting_grid(session_key: str = None):
    ...
    if not session_key:
        session_key = "latest"
    return await safe_cached_get(f"{OPENF1}/sstarting_grid?session_key={session_key}", [], ttl=3600)
...
async def overtakes(session_key: str = None):
    ...
    if not session_key:
        session_key = "latest"
    return await safe_cached_get(f"{OPENF1}/overtakes?session_key={session_key}", [], ttl=30)
...
async def session_mode():
    ...
    try:
        ...
        return {"mode": state.get("mode", "idle"), "ts": __import__("datetime").datetime.now(__import__("datetime").timezone.utc).isoformat()}
    except Exception:
