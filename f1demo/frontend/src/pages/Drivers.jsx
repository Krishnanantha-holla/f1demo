...
import { EmptyMsg, ErrorMsg, Loading } from "../components/Shared";

const ARVID_HEADSHOT =
  "https://media.formula1.com/content/dam/fom-website/drivers/2025Drivers/lindblad.png.transform/2col/image.png";

const CAR_YEAR = new Date().getFullYear();
const CAR_YEARS = [CAR_YEAR, CAR_YEAR - 1, CAR_YEAR - 2, CAR_YEAR - 3].filter(
  (y, i, arr) => y >= 2022 && arr.indexOf(y) === i,
);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        const driverLaps = Array.isArray(lapData)
          ? lapData.filter((l) => l.driver_number === driver.driver_number)
          : [];
        const driverPits = Array.isArray(pitData)
          ? pitData.filter((p) => p.driver_number === driver.driver_number)
          : [];
        const driverStints = Array.isArray(stintData)
          ? stintData.filter((s) => s.driver_number === driver.driver_number)
          : [];
        ...
        const s1 = driverLaps
          .filter((l) => l.duration_sector_1 > 0)
          .map((l) => l.duration_sector_1);
        const s2 = driverLaps
          .filter((l) => l.duration_sector_2 > 0)
          .map((l) => l.duration_sector_2);
        const s3 = driverLaps
          .filter((l) => l.duration_sector_3 > 0)
    ...
    // Fetch AI-generated driver summary (non-blocking) with localStorage cache
    (async () => {
      setAiLoading(true);
      setAiError(null);
      try {
        ...
        if (!name) { setAiLoading(false); return; }

        const cacheKey = `ai_summary:driver:${name.replace(/\s+/g, '_')}`;
        try {
          const raw = localStorage.getItem(cacheKey);
          if (raw) {
            const parsed = JSON.parse(raw);
            if (parsed && parsed.e && Date.now() < parsed.e) {
              if (!cancelled) { setAiSummary(parsed.v); setAiLoading(false); }
              return;
            }
          }
        } catch { /* ignore */ }

        const res = await api.aiSummary("driver", name);
        if (!cancelled && res && res.summary) {
          setAiSummary(res.summary);
          try { localStorage.setItem(cacheKey, JSON.stringify({ v: res.summary, e: Date.now() + 1000 * 60 * 60 * 12 })); } catch {}
        }
        if (!cancelled && res?.ai_fallback) {
          setAiError("Grok AI not configured — showing cached bio");
        }
      } catch (e) {
        console.warn("[PilotProfile] AI summary failed", e);
        if (!cancelled) setAiError("Grok AI not configured — showing cached bio");
      } finally {
      ...
      <div className={`encyclopedia-panel ${closing ? "closing" : ""}`}>
        ...
        <div className="panel-header">
          ...
          <div className="panel-title-area">
            ...
            <h3 className="panel-team" style={{ color: teamColor }}>
              {driver.team_name}
            </h3>
          </div>
        </div>
        ...
        <div className="panel-scroll-content">
          <div className="bio-section">
            <p className="bio-text">
              {aiSummary ? aiSummary : shortBioText(driver, info.bio)}
            </p>
            {aiLoading && (
            ...
            {aiError && (
              <div style={{ marginTop: "0.5rem", color: "var(--f1-red)" }}>
          ...
          <div className="stats-showcase">
            <div className="stat-hero">
              <span className="stat-hero-val">
                {statValueOrDash(info.championships)}
              </span>
              <span className="stat-hero-label">World Titles</span>
            </div>
            <div className="stat-hero">
              <span className="stat-hero-val">
                {statValueOrDash(info.wins)}
              </span>
              <span className="stat-hero-label">Wins</span>
            </div>
            <div className="stat-hero">
              <span className="stat-hero-val">
                {statValueOrDash(info.podiums)}
              </span>
          ...
          <div className="stats-grid-modern">
            ...
            <div className="stat-box">
              <span className="box-val" style={{ color: teamColor }}>
                {standing?.points ?? "—"}
              </span>
          ...
          {detail && !detail.error && (detail.bestS1 || detail.bestS2) && (
            <>
              ...
              <div className="stats-grid-modern">
                <div className="stat-box">
                  <span className="box-val">
                    {detail ? fmtLap(detail.bestS1) : "—"}
                  </span>
                  <span className="box-label">Best S1</span>
                </div>
                <div className="stat-box">
                  <span className="box-val">
                    {detail ? fmtLap(detail.bestS2) : "—"}
                  </span>
                  <span className="box-label">Best S2</span>
                </div>
                <div className="stat-box">
                  <span className="box-val">
                    {detail ? fmtLap(detail.bestS3) : "—"}
                  </span>
                  ...
                  <div className="modal-lap-mini">
                    {detail.tyres.map((t, i) => {
                      ...
                      return (
                        <div
                          key={`${t}-${i}`}
                          className="modal-lap-chip"
                          style={{
                            borderColor: c[t.toUpperCase()] || "var(--border)",
                          }}
                        >
                          <span
                            style={{
                              color: c[t.toUpperCase()] || "var(--text)",
                              fontWeight: 700,
                            }}
                          >
                            {t}
                          </span>
                          <span className="modal-lap-chip-label">
                            Stint {i + 1}
                          </span>
                        </div>
                      );
              ...
              {teammate && tmStanding && (
                <div className="modal-section">
                  ...
                  <div className="h2h-grid">
                    <div className="h2h-driver" style={{ color: teamColor }}>
                      {driver.full_name}
                    </div>
                    <div className="h2h-vs">VS</div>
                    <div className="h2h-driver" style={{ textAlign: "right" }}>
                      {teammate.full_name}
                    </div>
                  </div>
                </div>
...
export default function Drivers() {
  ...
  const [expandedDriver] = useState(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [driverData, freeRoster, standingsData, bioData] =
          await Promise.all([
            api.drivers().catch((err) => {
              console.warn("[Drivers] drivers fetch failed", err);
              return [];
            }),
            api.freeRoster().catch((err) => {
              console.warn("[Drivers] freeRoster fetch failed", err);
              return [];
            }),
            api.driverStandings().catch((err) => {
              console.warn("[Drivers] standings fetch failed", err);
              return [];
            }),
            api.bios().catch((err) => {
              console.warn("[Drivers] bios fetch failed", err);
              return { drivers: {} };
            }),
          ]);
        ...
        (freeRoster || []).forEach((d) =>
          merged.set(d.driver_number, { ...d }),
        );
  ...
  return (
    <>
      <div className="page-header">
        <div>
          ...
          <div className="page-subtitle">
            Season roster with resilient coverage across live and archived data
            feeds.
          </div>
        </div>
      ...
      {status === "empty" && (
        <EmptyMsg text="Driver roster is temporarily unavailable." />
      )}
      {status === "ok" && (
        <div className="drivers-grid">
          {drivers.map((d, i) => {
            ...
            const teammate =
              drivers.find(
                (td) =>
                  td.team_name === d.team_name &&
                  td.driver_number !== d.driver_number,
              ) || null;
            const teammatePoints = teammate
              ? standings[teammate.driver_number]?.points || 0
              : null;
            return (
              <div
                key={d.driver_number}
                ...
                onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setModalDriver(d.driver_number); } }}
                style={{
              ...
              >
                  ...
                  <div className="driver-card-info">
                    ...
                    {s && (
                      <div style={{ marginTop: "0.35rem", fontSize: "0.8rem" }}>
                        <span
                          style={{ color: "var(--f1-red)", fontWeight: 700 }}
                        >
                          {!isNaN(s.position) ? `P${s.position}` : "—"}
                        </span>
                        <span
                          style={{
                ...
                <div className="detail-expand">
                  <div className="detail-stats">
                    <div className="stat-box">
                      <div className="stat-val">
                        {statValueOrDash(bio.championships)}
                      </div>
                      <div className="stat-label">Titles</div>
                    </div>
                    <div className="stat-box">
                      <div className="stat-val">
                        {statValueOrDash(bio.wins)}
                      </div>
                      <div className="stat-label">Career Wins</div>
                    </div>
                    <div className="stat-box">
                      <div className="stat-val">{statValueOrDash(s?.wins)}</div>
                  ...
                  <div className="detail-laps">
                    ...
                    <div className="detail-lap-row">
                      <span>Career Tier</span>
                      <strong>{legacy}</strong>
                    </div>
                    <div className="detail-lap-row">
                      <span>Career Podiums</span>
                      <strong>{bio.podiums ?? "—"}</strong>
                    </div>
                    <div className="detail-lap-row">
                      <span>Career Points</span>
                      <strong>{bio.career_points ?? "—"}</strong>
                    </div>
                    <div className="detail-lap-row">
                      <span>Teammate</span>
                      <strong>
                        {teammate
                          ? teammate.name_acronym || teammate.last_name
                          : "TBD"}
                      </strong>
                    </div>
                    <div className="detail-lap-row">
                      <span>Garage Delta</span>
                      <strong>
                        {teammatePoints != null && s
                          ? `${s.points - teammatePoints >= 0 ? "+" : ""}${(s.points - teammatePoints).toFixed(0)} pts`
                          : "—"}
                      </strong>
                    </div>
                  </div>

                  <div className="driver-inline-bio">
                    {shortBioText(d, bio.bio)}
                  </div>
